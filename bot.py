...
user_states = {}
admin_panel_msgs = {} # Tracks the Admin Panel Message ID to update in-place
...
def update_admin_panel(chat_id, text, markup):
    ...
    if msg_id:
        try:
            ...
            pass
    # If fails or doesn't exist, send fresh
    msg = bot.send_message(chat_id, text, reply_markup=markup, parse_mode="Markdown")
    admin_panel_msgs[chat_id] = msg.message_id

def show_main_admin_menu(chat_id):
    user_states.pop(chat_id, None) # Clear any pending states
    markup = InlineKeyboardMarkup()
    ...
    markup.row(InlineKeyboardButton(f"⏱️ Set Auto-Broadcast Time ({bc_mins}m)", callback_data="adm_set_bc_time"))
    ...
    text = "👑 **Admin Control Panel**\n\nWelcome! Please choose an option below to configure your bot. Everything will update cleanly right here."
...
def handle_callbacks(call):
    ...
    # --- USER CALLBACKS (Remains Untouched) ---
    if data == "back_home":
        try: bot.delete_message(user_id, msg_id)
        except: pass
    ...
    # --- ADMIN ACTIONS (Single Message Updates) ---
    if user_id == ADMIN_ID:
        # Start Videos Menu
        if data == "adm_start_vids_menu":
            markup = InlineKeyboardMarkup()
            markup.row(InlineKeyboardButton("➕ Add Start Videos (Multi-Upload)", callback_data="adm_add_start_vid"))
        ...
        # Payment Validation Actions (Fixed)
...
def handle_all_inputs(message):
    ...
    if user_id == ADMIN_ID:
        # Delete Admin's message to keep chat clean
        if state:
